...
class RenPyTests(LargeFrameworkTests):
    """This will test the Ren'Py installation"""
    ...
    def setUp(self):
        ...
        self.desktop_filename = "renpy.desktop"

    def test_default_stencyl_install(self):
        """Install renpy from scratch test case"""
        self.child = spawn_process(self.command('{} games renpy'.format(UMAKE)))
        self.expect_and_no_warn("Choose installation path: {}".format(self.installed_path))
        self.child.sendline("")
        self.expect_and_no_warn("Installation done", timeout=self.TIMEOUT_INSTALL_PROGRESS)
        self.wait_and_close()
        ...
        # ensure that it's detected as installed:
        self.child = spawn_process(self.command('{} games renpy'.format(UMAKE)))
        self.expect_and_no_warn("Ren'Py is already installed.*\[.*\] ")
        self.child.sendline()
        self.wait_and_close()
