...
class BaseEclipse(umake.frameworks.baseinstaller.BaseInstaller, metaclass=ABCMeta):
    ...
    def parse_download_link(self, line, in_download):
        ...
        if in_download:
            ...
            with suppress(AttributeError):
                self.sha512_url = 'https://www.eclipse.org/downloads/sums.php?file=' + parse.quote(p.group(1), safe='') + '&type=sha512'
                url_found = True
    ...
    def get_sha_and_start_download(self, download_result):
        ...
        url = re.sub('https://www\.eclipse\.org/downloads/sums\.php\?file=(.*)&type=sha512', 'http://www.eclipse.org/downloads/download.php?file=\g<1>&mirror_id=1', self.sha512_url)
        if url is None:
